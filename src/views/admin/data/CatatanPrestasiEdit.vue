...
const doStoreData = async (d) => {
  ...
  try {
    const response = await Api.put(
      `ortu/data/catatan/prestasi/data/${id}`,
      dataStore
    );
...
const getDataId = async () => {
  try {
    const response = await Api.get(`ortu/data/catatan/prestasi/${id}`);
...
<template>
  <div class="pt-4 px-10 md:flex justify-between">
    ...
    <div class="md:py-0 py-4">
      <BreadCrumb>
        <template v-slot:content> Prestasi <BreadCrumbSpace /> Edit </template>
      </BreadCrumb>
    </div>
  </div>
  <div class="pt-4 px-10 md:flex justify-between">
